...
function openNavigation(q,e){
  ...
  if(!isiOS) return;
  e.preventDefault();
  const scheme=`comgooglemaps://?daddr=${encodeURIComponent(q)}&directionsmode=driving`;
...
if('serviceWorker' in navigator) window.addEventListener('load',async()=>{
  try{
    const reg=await navigator.serviceWorker.register('./service-worker.js?v=6.3',{updateViaCache:'none'});
