const TRIP = {
  ...
  days:[
    {date:'2026-11-05',label:'DAY 1',title:'成田抵達・自駕前往箱根',subtitle:'桃園 → 成田 → 海螢火蟲 → 小田原 → 箱根',page:7,routePage:8,image:'https://pca.jp/p-tips/upload/assets/2025030514395139.jpg',events:[
      ['04:30','桃園機場 T1','集合',null],['06:50','Scoot TR898','TPE → NRT',null],['10:55','成田國際機場 T1','抵達','成田国際空港 第1ターミナル'],['','NIPPON Rent-A-Car','取車','NIPPON Rent-A-Car 成田空港'],['','海螢火蟲 PA','午餐・休息','海ほたるPA'],['','MaxValu 小田原荻窪店','採買','マックスバリュ小田原荻窪店'],['','Noel Hakone Gensen','入住箱根民宿','Noel Hakone Gensen']
    ]},
    ...
    {date:'2026-11-07',label:'DAY 3',title:'箱根療癒・前導東京',subtitle:'箱根散策 → 小田原還車 → 赤坂',page:15,routePage:16,extraPage:17,image:'assets/hero/day3.webp',events:[
      ['10:00','箱根民宿','退房','Noel Hakone Gensen'],['','箱根雕刻之森美術館','參觀','彫刻の森美術館'],['','箱根空中纜車','移動／景觀','箱根ロープウェイ'],['','蘆之湖','散策','芦ノ湖'],['','箱根神社','參拜','箱根神社'],['15:00','小田原','還車後搭車前往東京','小田原駅'],['晚間','OMO3 東京赤坂','入住','OMO3 東京赤坂 by 星野リゾート']
    ]},
...
function renderBook(){view.innerHTML=`<section><button class="back" data-more="root">‹ 返回</button><div class="section-head"><h2>完整旅遊手冊</h2><small>27 頁</small></div><a class="secondary full link-button book-pdf-link" target="_blank" rel="noopener" href="assets/travel-book.pdf"><span><strong>開啟高畫質原始 PDF ↗</strong><small>高畫質・完整 27 頁</small></span></a><p class="note">這裡已換成你最新的旅遊手冊 V3，方便旅行中快速翻頁。點任何頁面可放大。</p><div class="book-grid">${pageMeta.map(([n,t])=>`<div class="book-item">${pageImg(n)}<small>${String(n).padStart(2,'0')}｜${t}</small></div>`).join('')}</div></section>`;bind();}
function renderMore(mode='root'){
  if(mode==='flight')return renderFlight(); if(mode==='stay')return renderStay(); if(mode==='check')return renderSub('行李檢查表',`${checklist()}<div class="section">${pageImg(27)}</div>`); if(mode==='links')return renderLinks(); if(mode==='akasaka')return renderAkasaka(); if(mode==='spots')return renderSpots(); if(mode==='shopping')return renderShopping(); if(mode==='pretrip')return renderPretrip(); if(mode==='book')return renderBook();
  view.innerHTML=`<section><div class="section-head"><h2>更多資訊</h2><small>MORE</small></div><div class="list"><button class="list-btn" data-more="flight"><span><strong>✈ 航班資訊</strong><small>TR898 / TR875</small></span><b>›</b></button><button class="list-btn" data-more="stay"><span><strong>▰ 住宿與房間分配</strong><small>箱根民宿・OMO3 東京赤坂</small></span><b>›</b></button><button class="list-btn" data-more="links"><span><strong>◎ 實用連結</strong><small>Visit Japan Web・JNTO・影片教學</small></span><b>›</b></button><button class="list-btn" data-more="akasaka"><span><strong>⌖ 赤坂周邊</strong><small>日枝神社・紀尾井町・弁慶橋・豐川稻荷</small></span><b>›</b></button><button class="list-btn" data-more="spots"><span><strong>★ 東京景點</strong><small>經典景點候選</small></span><b>›</b></button><button class="list-btn" data-more="shopping"><span><strong>▣ 東京必買</strong><small>零食・藥妝・生活雜貨・伴手禮</small></span><b>›</b></button><button class="list-btn" data-more="check"><span><strong>☑ 行李檢查表</strong><small>可勾選並保留狀態</small></span><b>›</b></button><button class="list-btn" data-more="book"><span><strong>☷ 完整旅遊手冊</strong><small>27 頁全部放進 App</small></span><b>›</b></button></div></section>`;bind();}
...
if('serviceWorker' in navigator) window.addEventListener('load',async()=>{
  try{
    const reg=await navigator.serviceWorker.register('./service-worker.js?v=5.4',{updateViaCache:'none'});
    await reg.update();
