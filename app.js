...
${TRIP.days.map((x,i)=>`<button class="journey-card" data-open-day="${i}"><img src="${x.image}" alt="${x.title}"><span class="journey-copy"><small>${fmt(x.date)}</small><strong>${x.label}</strong><em>${x.title}</em></span><b>›</b></button>`).join('')}
</section>
<section class="home-stays">
 <button class="stay-glass" data-more="stay"><img src="assets/hakone-stay.webp" alt="箱根民宿"><span><small>11/05 — 11/07・2 晚</small><strong>箱根民宿</strong><em>Noel Hakone Gensen</em></span><b>›</b></button>
 <button class="stay-glass" data-more="stay"><img src="assets/omo3-stay.webp" alt="OMO3 東京赤坂"><span><small>11/07 — 11/09・2 晚</small><strong>OMO3 東京赤坂</strong><em>by 星野リゾート</em></span><b>›</b></button>
</section>
<section class="home-mini-actions"><button data-more="flight">✈<span>航班</span></button><button data-view-link="map">⌖<span>導航</span></button><button data-more="check">✓<span>行李</span></button><button data-more="links">↗<span>連結</span></button></section>
...
if('serviceWorker' in navigator) window.addEventListener('load',async()=>{
  try{
    const reg=await navigator.serviceWorker.register('./service-worker.js?v=2.1',{updateViaCache:'none'});
    await reg.update();
